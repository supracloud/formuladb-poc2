...
if ! (command nodemon || command wait-on); then
     echo "run npm install -g nodemon wait-on in order to serve/debug this app";
     exit 1;
fi

FRMDB_RELEASE=0.0.12 nodemon --verbose --delay 200ms --watch docker-img -e t --exec "docker-compose up -d db febe maps && wait-on tcp:8084 && wait-on tcp:8085 && touch docker-img/compose.t" &

sleep 1

cd formuladb && live-server --watch=apps,docker-img/compose.t \
     --entry-file=index.html --wait=200 --port=8081 -V \
