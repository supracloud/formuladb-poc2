...
export async function createNewEnvironment(envName: string, email: string, password:string) {
    ...
    if (!envName.match(/[-a-z]+/)) throw new Error(`env name ${envName} must contain only lowercase letters and dash`);
...
export async function cleanupEnvironment(envName: string) {
    ...
    // Delete git remote branch for formuladb-env/frmdb-apps. Thus, make sure we have the local git clone available and in sync with remote
    await exec(`mkdir -p env_workspace/${envName}`);
    await exec(`FRMDB_ENV_NAME=${envName} bash /scripts/prepare-env.sh`,
    {cwd: `env_workspace/${envName}`});
    await exec(`git push origin --delete ${envName} || true`,
    {cwd: `env_workspace/${envName}/formuladb-env/frmdb-apps`});
