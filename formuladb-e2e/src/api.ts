...
function _scenario(focusOnThisScenario: boolean, text: string, callback: () => void) {
    (focusOnThisScenario ? fdescribe : describe)(text, callback);
}

export function Scenario(text: string, callback: () => void) {
    _scenario(false, text, callback);
}

export function ScenarioFocused(text: string, callback: () => void) {
    _scenario(true, text, callback);
}
export function FullTestWithVideo(text: string, callback: () => void) {
    ...
    Scenario(text, callback);
}

function _step(focusOnThisStep: boolean, text: string, callback: () => Promise<any>) {
...
async function find(selector: string, parent?: ElementFinder): Promise<ElementFinder> {
    ...
    let tokens = selector.split(specialTokenRegex);
    let selectors: string[] = [];
    for (let [i, token] of tokens.entries()) {
        if (token.match(/^\s*$/)) continue;
        let prevToken = i == 0 ? '' : tokens[i - 1];
        if ("::parent" == token) {
            if (prevToken) selectors.push(prevToken);
