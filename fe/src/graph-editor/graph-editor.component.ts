...
const CSS: string = require('!!raw-loader!sass-loader?sourceMap!@fe-assets/graph-editor/graph-editor.component.scss').default;
...
class GraphEditor extends HTMLElement {
    ...
    init() {
        ...
        const nodes = [
            { id: 0, text: "NEW", reflexive: false },
            { id: 1, text: "PENDING", reflexive: false },
            { id: 2, text: "FINALIZED", reflexive: false },
            { id: 3, text: "CANCELLED", reflexive: false }
        ];
        let lastNodeId = 2;
        const links = [
        ...
        d3.select(this)//FIXME: only if binding to window we receive key events, why?
          .on('keydown', keydown)
          .on('keyup', keyup);
...
window.customElements.define('frmdb-graph-editor', GraphEditor);
