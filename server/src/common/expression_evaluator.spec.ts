...
describe('ExecutionPlanRunner', () => {
    ...
    it('should evaluate expressions correctly', () => {
        ...
        expect(exprEval.evaluate('$._id', { $: { _id: 123 } })).to.equal(123);
        expect(exprEval.evaluate('"a" + "b"', {})).to.eql("ab");
        expect(exprEval.evaluate('./gigi/gogu - 1', { $: { gigi: { gogu: 123 } } })).to.equal(122);
        expect(exprEval.evaluate("DATE_UTILS(t, START_OF_MONTH)", { t: '2018-02-12' })).to.equal('2018-02-01-00-00-00-000');
        expect(exprEval.evaluate("DATE_UTILS(t, END_OF_MONTH)", { t: '2018-02-12-22-01-59-123' })).to.equal('2018-02-28-23-59-59-999');
