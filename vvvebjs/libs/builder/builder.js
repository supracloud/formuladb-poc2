...
Vvveb.Builder = {
	...
	selectNode: function (node) {
		...
		window.frmdbEditorHighlightDataGridCell(node[0] && node[0].tagName ? node[0] : node);
	...
	_initHighlight: function () {
		...
		self.frameHtml.on("dblclick", function (event) {
			...
			if (Vvveb.Builder.isPreview == false) {
				...
				self.texteditEl.on("blur keyup paste input", function (event) {
					...
					let nodeLanguage = jQuery(self.texteditEl).attr('data-i18n');
					const nodeValue = jQuery(self.texteditEl).text();
					if (!nodeLanguage) {
		...
		self.frameHtml.on("click", function (event) {
			...
			if (Vvveb.Builder.isPreview == false) {
				...
				// image grabbing
				const imgUrl = self._grabImage(event.target);
				if (imgUrl)
	...
	_grabImage(element) {
		...
		for (el of element.querySelectorAll("div")) {
...
Vvveb.Gui = {
	...
	init: function () {
		...
		const currentLanguage = i18n.getLangDesc(localStorage.getItem('editor-lang') || i18n.defaultLanguage);
